 ...
 */
@Service
@RequiredArgsConstructor   // 含有final的就是必须构造的，不是final就不是
...
//        List<Cart> carts = lambdaQuery().eq(Cart::getUserId, UserContext.getUser()).list();
    ...
    private void handleCartItems(List<CartVO> vos) {
        ...
        Optional<List<ItemDTO>> itemDTOS = Optional.ofNullable(itemClient.queryItemByIds(itemIds));
        if(itemDTOS.isEmpty()) {
            return;
        }
        List<ItemDTO> items = itemDTOS.get();
